...
class Manager {
    ...
    getInternalState() {
        return {acc: this.acc, pc: this.pc, z: this.z, n: this.n, inst: this.inst, end: this.end}
    }

    resetProgramMemoryAndRegisters() {
        this.memory = this.memory.fill(0, 128, 255)
        this.items = []
        this.pc = 0
        this.acc = 0
        this.z = 1
    ...
    _getCurrentStatus() {
        return {acc: Uint8Array.of(this.acc)[0], pc: this.pc, memory: this.memory, end: this.end, z: this.z, n: this.n, inst:this.inst}
    }

    _getStatusResponse() {
        return {acc: this.acc, pc: this.pc, memory: this.memory, z: this.z, n: this.n,end: this.end, inst: this.inst}
    }
    ...
    _undo() {
        ...
        const {acc, pc, memory, end, z, n, inst} = this.items.pop()
        this.acc = acc
        this.pc = pc
        this.memory = memory
        this.end = end
    ...
    _jmp() {
        ...
        this.pc = this._getRawFromMemory()
    }
